...
import demos.kdtree.RangeSearch;
 ...
 * @author benland100
 */
public class RangeSearchApplet extends JApplet {

    public static void main(String[] args) {
        JFrame frame = new JFrame("kdTree Range Search Demo");
        frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        frame.setSize(500, 500);
        frame.add(new RangeSearchApplet());
        frame.setVisible(true);
    }

    private RangeSearch searcher;

    public RangeSearchApplet() {
        searcher = new RangeSearch();
        searcher.setRange(50);
        ...
        final JSlider range = new JSlider(JSlider.HORIZONTAL, 1, 1000, 50);
        final JLabel rangelbl = new JLabel("50");
        range.addChangeListener(new ChangeListener() {

            public void stateChanged(ChangeEvent e) {
                rangelbl.setText(Integer.toString(range.getValue()));
                searcher.setRange(range.getValue());
                searcher.recalc();
                searcher.repaint();
            }
        });
        range.setSnapToTicks(true);
        ...
        left.add(new JLabel(" Range to Search "));
        center.add(range);
        right.add(rangelbl);
